...
from vision2step.geometry_hints import GeometryHints, extract_geometry_hints
...
class VisionAnalyzer:
    ...
    def analyze(
        ...
        *,
        ...
        measurements = self._measure_images(images)
        content = self._build_content(images, measurements, object_context)
        try:
        ...
        return AnalysisArtifact(
            ...
            geometry_hints=[
                hints.as_record(index, images[index - 1].path.name)
                for index, hints in measurements
            ],
    ...
    def _build_content(
        ...
        measurements: Sequence[tuple[int, GeometryHints]],
        object_context: str,
    ) -> list[dict[str, Any]]:
        ...
        measurement_reports = [hints.as_prompt_text(index) for index, hints in measurements]
    ...
    @staticmethod
    def _measure_images(images: Sequence[EncodedImage]) -> list[tuple[int, GeometryHints]]:
        measurements: list[tuple[int, GeometryHints]] = []
        for index, image in enumerate(images, start=1):
            try:
            ...
            if hints is not None:
                measurements.append((index, hints))
        return measurements
